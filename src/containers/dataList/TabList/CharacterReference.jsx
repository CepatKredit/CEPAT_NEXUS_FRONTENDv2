...
function CharacterReference({ classname, BorrowerId, Creator, isEdit, User, data }) {
    ...
    const getCharacterRef = useQuery({
        ...
        queryFn: async () => {
            try {
                const result = await axios.get(`/api/GET/G13CR/${BorrowerId}`);
                let dataList = [{
                    key: 0,
                    no: '',
                    name: '',
    ...
    const getRelationshipList = useQuery({
        ...
        queryFn: async () => {
            const result = await axios.get('/api/GET/G12R');
    ...
    const provinceList = useQuery({
        ...
        queryFn: async () => {
            const result = await axios.get('/api/GET/G8P');
    ...
    const onClickSaveData = useMutation({
        mutationFn: async (row) => {
            ...
            try {
                const result = await axios.post('/api/POST/P60ACR', data);
    ...
    const onClickEditData = useMutation({
        mutationFn: async (row) => {
            ...
            await axios.post('/api/POST/P61UCR', data)
                .then((result) => {
                    api[result.data.status]({
                        message: result.data.message,
                        description: result.data.description,
    ...
    const onClickDeteleData = useMutation({
        mutationFn: async (e) => {
            try {
                const result = await axios.post(`/api/POST/P62DCR/${e}`);
                queryClient.invalidateQueries({ queryKey: ['getCharacterRef'] }, { exact: true });
                api[result.data.status]({
                    message: result.data.message,
                    description: result.data.description
