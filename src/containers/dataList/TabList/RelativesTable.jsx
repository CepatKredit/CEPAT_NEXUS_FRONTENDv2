...
function Relatives({ BorrowerId, onUpdateCount, User, data }) {
    ...
    const getRelatives = useQuery({
        ...
        queryFn: async () => {
            try {
                const result = await axios.get(`/api/GET/G35R/${BorrowerId}`);
                let dataList = [{
                    key: 0,
                    no: '',
                    FullName: '',
    ...
    const getRelationshipList = useQuery({
        ...
        queryFn: async () => {
            const result = await axios.get('/api/GET/G33RR');
    ...
    const onClickSaveData = useMutation({
        mutationFn: async (row) => {
            ...
            try {
                console.log(data)
                const result = await axios.post('/api/POST/P75AR', data);
    ...
    const onClickEditData = useMutation({
        mutationFn: async (row) => {
            try {
                ...
                console.log(data)
                const result = await axios.post('/api/POST/P76UR', data);
                api[result.data.status]({
                    message: result.data.message,
                    description: result.data.description,
                });
    ...
    const onClickDeleteData = useMutation({
        mutationFn: async (e) => {
            try {
                const result = await axios.post(`/api/POST/P77DR/${e}`);
                queryClient.invalidateQueries({ queryKey: ['getRelatives'] }, { exact: true });
                api[result.data.status]({
                    message: result.data.message,
                    description: result.data.description
