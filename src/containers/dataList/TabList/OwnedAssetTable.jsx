...
function OwnedAsset({ data, User }) {
    ...
    const getOwnedAssets = useQuery({
        ...
        queryFn: async () => {
            ...
            try {
                const result = await axios.get(`/v1/GET/G110OA/${toDecrypt(localStorage.getItem('SIDC'))}`);
                let dataList = [{
                    key: 0,
                    no: '',
                    Category: '',
    ...
    async function onClickSave() {
        ...
        await axios.post('/v1/POST/P133AOA', data)
            .then((result) => {
    ...
    async function onClickDelete(e) {
        try {
            const result = await axios.post(`/v1/POST/P135DOA/${e}`);
            queryClient.invalidateQueries({ queryKey: ['getOwnedAssets'] }, { exact: true });
            api[result.data.status]({
                message: result.data.message,
                description: result.data.description
