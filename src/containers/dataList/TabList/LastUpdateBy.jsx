    ...
    const onClickUpdateStatus = useMutation({
        mutationFn: async () => {
            ...
            if (getUpdate.Status === 'PRE-CHECK' || getUpdate.Status === 'FOR APPROVAL') {
                ...
                try {
                    await axios.post('/v1/POST/P139UCA', checkListData);
                    api['success']({
                ...
                if (getUpdate.UrgentApp === 2 && getUpdate.DepartureDate) {
                    const departureUpdateResponse = await axios.post('/v1/POST/P140UD', {
                        LoanAppId: toDecrypt(localStorage.getItem('SIDC')),
                ...
                if (getUpdate.Status === 'FOR DISBURSEMENT') {
                    await axios.post('/v1/POST/P141UD', dataContainer[0])
                ...
                if (checkpn !== undefined) {
                    ...
                    try {
                        const result = await axios.post('/v1/POST/P81UAS',
