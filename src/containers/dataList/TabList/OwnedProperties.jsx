...
function OwnedProperties({ data, User }) {
    ...
    const getOwnedProperties = useQuery({
        ...
        queryFn: async () => {
            ...
            try {
                const result = await axios.get(`/v1/GET/G111OP/${toDecrypt(localStorage.getItem('SIDC'))}`);
                let dataList = [{
                    key: 0,
                    no: '',
                    Properties: '',
    ...
    async function onClickSave() {
        ...
        console.log(data)
        await axios.post('/v1/POST/P136AOP', data)
            .then((result) => {
    ...
    async function onClickEdit() {
        ...
        try {
            ...
            const result = await axios.post('/v1/POST/P137UOP', data);
            api[result.data.status]({
                message: result.data.message,
                description: result.data.description,
            });
    ...
    async function onClickDelete(e) {
        try {
            const result = await axios.post(`/v1/POST/P138DOP/${e}`);
            queryClient.invalidateQueries({ queryKey: ['getOwnedProperties'] }, { exact: true });
            api[result.data.status]({
                message: result.data.message,
                description: result.data.description
