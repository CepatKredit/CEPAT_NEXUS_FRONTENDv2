...
function OtherLoanHistory({ data, User }) {
    ...
    const getOtherLoanHistory = useQuery({
        ...
        queryFn: async () => {
            ...
            try {
                const result = await axios.get(`/api/GET/G9OLH/${toDecrypt(localStorage.getItem('SIDC'))}`);
    ...
    async function onClickSave() {
        ...
        //   console.log(data)
        await axios.post('/api/POST/P55ALH', data)
            .then((result) => {
    ...
    async function onClickEdit() {
        try {
            ...
            const result = await axios.post('/api/POST/P56UOLH', data);
            api[result.data.status]({
                message: result.data.message,
                description: result.data.description,
            });
    ...
    async function onClickDelete(e) {
        try {
            const result = await axios.post(`/api/POST/P57DOLH/${e}`);
            queryClient.invalidateQueries({ queryKey: ['getOtherLoanHistory'] }, { exact: true });
            api[result.data.status]({
                message: result.data.message,
                description: result.data.description
