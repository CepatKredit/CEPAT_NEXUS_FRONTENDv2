...
function EmploymentHistory({ data, User }) {
    ...
    const getEmploymentHistory = useQuery({
        ...
        queryFn: async () => {
            ...
            try {
                const result = await axios.get(`/api/GET/G107EH/${toDecrypt(localStorage.getItem('SIDC'))}`);
    ...
    async function onClickSave() {
        ...
        await axios.post('/api/POST/P128AEH', data)
            .then((result) => {
    ...
    async function onClickEdit() {
        ...
        try {
            ...
            const result = await axios.post('/api/POST/P127UEH', data);
            api[result.data.status]({
                message: result.data.message,
                description: result.data.description,
            });
    ...
    async function onClickDelete(e) {
        try {
            const result = await axios.post(`/api/POST/P130DEH/${e}`);
            queryClient.invalidateQueries({ queryKey: ['getEmploymentHistory'] }, { exact: true });
            api[result.data.status]({
                message: result.data.message,
                description: result.data.description
