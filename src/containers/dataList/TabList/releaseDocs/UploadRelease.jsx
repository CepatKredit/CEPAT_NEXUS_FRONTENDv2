...
function DocxTable({ showModal, closeModal, Display, docTypeList, ClientId, Uploader, FileType, LoanStatus }) {
    React.useEffect(() => { console.log(ClientId + 'UploadRelease') }, [ClientId])
    ...
    function GetDocsCode(data) {
        ...
        const DocsCode = dataHolder.find((x) => GetStatus === 'FOR DOCUSIGN' || GetStatus === 'FOR DISBURSEMENT' || GetStatus === 'TAGGED FOR RELEASE'
            ? x.docsType === data || x.id === data && (x.name === 'LPACCDDT')
            : GetStatus === 'RELEASED'
                ? x.docsType === data || x.id === data && (x.name === 'LPACCREL')
                : x.docsType === data || x.id === data)
        return DocsCode.id
    }
    ...
    function DropDownSelection() {
        let container = []
        GetStatus === 'FOR DOCUSIGN' || GetStatus === 'FOR DISBURSEMENT' || GetStatus === 'TAGGED FOR RELEASE'
            ? docTypeList?.map((x) => { if (x.name === 'LPACCDDT') { container.push({ value: x.docsType, label: x.docsType }) } })
            : docTypeList?.map((x) => { if (x.name === 'LPACCREL') { container.push({ value: x.docsType, label: x.docsType }) } })
        return container
    }
                ...
                (<Form.Item style={{ margin: 0 }} name={dataIndex}
                    ...
                    {dataIndex === 'docxType'
                        ? (<Select ref={inputRef} value={fileList[record.key].status} onKeyDown={(e) => { if (e.key.toUpperCase() === 'ENTER') { save() } }}
                            onBlur={save} className='w-[100%]' options={DropDownSelection().map((x) => ({ value: x.value, label: x.label }))}
                        />)
    ...
    const onClickSaveFile = useMutation({
        mutationFn: async () => {
            ...
            let remarks_list = ''
            let docStatus_list = ''
            let prid = GetStatus === 'FOR DOCUSIGN' ||
                GetStatus === 'FOR DISBURSEMENT' ||
                GetStatus === 'TAGGED FOR RELEASE' ? 'LPACCDDT' : 'LPACCREL'
            fileList.map((x) => {
                if (status_list === '') {
