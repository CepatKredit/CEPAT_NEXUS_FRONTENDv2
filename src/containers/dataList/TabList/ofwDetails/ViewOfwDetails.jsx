...
function ViewOfwDetails({ data, User, RelativesCount, receive }) {
    ...
    const items = [
        ...
        { key: '3', label: <span className={`font-semibold ${data.ofwlname ? 'text-black' : 'text-red-600'}`}>Last Name</span>, children: data.ofwlname || '' },
        { key: '4', label: <span className={`font-semibold ${data.ofwsuffix ? 'text-black' : 'text-red-600'}`}>Suffix</span>, children: suffixOption?.find(suffix => suffix.code === data.ofwsuffix)?.description || '' },
        { key: '5', label: <span className={`font-semibold ${data.ofwbdate ? 'text-black' : 'text-red-600'}`}>Birthdate</span>, children: data.ofwbdate ? mmddyy(data.ofwbdate, 'MM-DD-YYYY') : '' },
        (User === 'Credit' || User === 'Lp') && { key: '6', label: <span className={`font-semibold ${data.ofwbdate ? 'text-black' : 'text-red-600'}`}>Age</span>, children: data.ofwbdate ? calculateAge(data.ofwbdate) : '' },
        { key: '7', label: <span className={`font-semibold ${data.ofwgender ? 'text-black' : 'text-red-600'}`}>Gender</span>, children: Gender().find(gender => gender.value === data.ofwgender)?.label || '' },
        { key: '8', label: <span className={`font-semibold ${data.ofwmobile ? 'text-black' : 'text-red-600'}`}>Mobile Number</span>, children: data.ofwmobile ? data.ofwmobile.replace('/', '-') : '' },
        ...
        (User === 'Credit' || User === 'Lp') && { key: '15', label: <span className={`font-semibold ${data.Religion ? 'text-black' : 'text-red-600'}`}>Religion</span>, children: Religion().find(ofwReligion => ofwReligion.value === data.Religion)?.label || '' },
    ...
    if (data.ofwresidences === 3) {
        items.push({
            key: '24',
            label: <span className={`font-semibold ${data.ofwrent ? 'text-black' : 'text-red-600'}`}>Rent Amount</span>,
        ...
        items.push({
            key: '24',
            label: <span className={`font-semibold ${data.ofwrent ? 'text-black' : 'text-red-600'}`}>Monthly Amortization</span>,
            children: data.ofwrent ? formatNumberWithCommas(formatToTwoDecimalPlaces(data.ofwrent.toString().replaceAll(',', ''))) : '',
        });
    }
    if (data.ofwmstatus === 2 || data.ofwmstatus === 5 || data.ofwmstatus === 6) {
        items.push(
            User !== 'LC' && { key: '19', label: <span className={`font-semibold ${data.ofwspouse ? 'text-black' : 'text-red-600'}`}>Spouse Name</span>, children: data.ofwspouse || '' },
            User !== 'LC' && { key: '20', label: <span className={`font-semibold ${data.ofwspousebdate ? 'text-black' : 'text-red-600'}`}>Spouse Birthdate</span>, children: data.ofwspousebdate ? mmddyy(data.ofwspousebdate) : '' },
            (User === 'Credit' || User === 'Lp') && { key: '21', label: <span className={`font-semibold ${data.SpSrcIncome ? 'text-black' : 'text-red-600'}`}>Spouse Source of Income</span>, children: SpouseSourceIncome().find(status => status.value === data.SpSrcIncome)?.label || '' },
            (User === 'Credit' || User === 'Lp') && { key: '22', label: <span className={`font-semibold ${data.SpIncome ? 'text-black' : 'text-red-600'}`}>Spouse Income</span>, children: formatNumberWithCommas(formatToTwoDecimalPlaces(data.SpIncome)).toString() },
        );
    }

    const addressItems = [
        { key: '25', label: <span className={`font-semibold ${data.ofwPresProvname ? 'text-black' : 'text-red-600'}`}>Present Area/Province</span>, children: data.ofwPresProvname || '' },
        { key: '26', label: <span className={`font-semibold ${data.ofwPresMunicipalityname ? 'text-black' : 'text-red-600'}`}>Present City/Municipality</span>, children: data.ofwPresMunicipalityname || '' },
        { key: '27', label: <span className={`font-semibold ${data.ofwPresBarangayname ? 'text-black' : 'text-red-600'}`}>Present Barangay</span>, children: data.ofwPresBarangayname || '' },
        { key: '28', label: <span className={`font-semibold ${data.ofwPresStreet ? 'text-black' : 'text-red-600'}`}>Present Street</span>, children: data.ofwPresStreet || '' },
        User !== 'LC' && { key: '29', label: <span className={`font-semibold ${data.landmark ? 'text-black' : 'text-red-600'}`}>Landmark</span>, children: data.landmark || '' },
        (User === 'Credit' || User === 'Lp') && { key: '30', label: <span className={`font-semibold ${data.OfwPoBRemarks ? 'text-black' : 'text-red-600'}`}>Proof of Billing Remarks</span>, children: data.OfwPoBRemarks || '' },
        User !== 'LC' && { key: '31', label: (<span className={`font-semibold ${data.ofwlosMonth || data.ofwlosYear ? 'text-black' : 'text-red-600'}`}> Length of Stay </span>), children: `${data.ofwlosYear > 0 ? `${data.ofwlosYear} Year(s)` : ``} ${(data.ofwlosYear > 0 && data.ofwlosMonth >= 1) ? ' / ' : ``} ${data.ofwlosMonth > 0 ? `${data.ofwlosMonth} Month(s)` : ``}` },
        User !== 'LC' && { key: '32', label: <span className={`font-semibold ${data.collectionareaname ? 'text-black' : 'text-red-600'}`}>Collection Area</span>, children: data.collectionareaname || '' },
        User !== 'LC' && { key: '33', label: <span className={`font-semibold ${data.ofwPermProvname ? 'text-black' : 'text-red-600'}`}>Permanent Area/Province</span>, children: data.ofwPermProvname || '' },
        User !== 'LC' && { key: '34', label: <span className={`font-semibold ${data.ofwPermMunicipalityname ? 'text-black' : 'text-red-600'}`}>Permanent City/Municipality</span>, children: data.ofwPermMunicipalityname || '' },
        User !== 'LC' && { key: '35', label: <span className={`font-semibold ${data.ofwPermBarangayname ? 'text-black' : 'text-red-600'}`}>Permanent Barangay</span>, children: data.ofwPermBarangayname || '' },
        User !== 'LC' && { key: '36', label: <span className={`font-semibold ${data.ofwPermStreet ? 'text-black' : 'text-red-600'}`}>Permanent Street</span>, children: data.ofwPermStreet || '' },
        User !== 'LC' && { key: '37', label: <span className={`font-semibold ${data.ofwprovProvname ? 'text-black' : 'text-red-600'}`}>Provincial Area/Province</span>, children: data.ofwprovProvname || '' },
        User !== 'LC' && { key: '38', label: <span className={`font-semibold ${data.ofwprovMunicipalityname ? 'text-black' : 'text-red-600'}`}>Provincial City/Municipality</span>, children: data.ofwprovMunicipalityname || '' },
        User !== 'LC' && { key: '39', label: <span className={`font-semibold ${data.ofwprovBarangayname ? 'text-black' : 'text-red-600'}`}>Provincial Barangay</span>, children: data.ofwprovBarangayname || '' },
        User !== 'LC' && { key: '40', label: <span className={`font-semibold ${data.ofwprovStreet ? 'text-black' : 'text-red-600'}`}>Provincial Street</span>, children: data.ofwprovStreet || '' },
    ];
    ...
    const idItems = [
        ...
        User !== 'LC' && { key: '43', label: <span className={`font-semibold ${data.ofwcountry ? 'text-black' : 'text-red-600'}`}>Country of Employment</span>, children: GET_COUNTRY_LIST?.find(country => country.code === data.ofwcountry || country.label === data.ofwcountry)?.description || '' },
        User !== 'LC' && {
        ...
        // User === 'Credit' && { key: '48', label: <span className={`font-semibold ${data.SpIncome ? 'text-black' : 'text-red-600'}`}>Contact Person</span>, children: data.ofwstatus || '' },
        // User === 'Credit' && { key: '49', label: <span className={`font-semibold ${data.SpIncome ? 'text-black' : 'text-red-600'}`}>Designation</span>, children: data.designation || '' },
        (User === 'Credit' || User === 'Lp') && { key: '47', label: <span className={`font-semibold ${data.FCurrency ? 'text-black' : 'text-red-600'}`}>Foreign Currency</span>, children: data.FCurrency ? `${GET_CURRENCY_LIST?.find(idtype => idtype.currencyCode === data.FCurrency)?.currencyName}` : '' },
        ...
        (User === 'Credit' || User === 'Lp') && { key: '57', label: <span className={`font-semibold ${data.AllotAmount ? 'text-black' : 'text-red-600'}`}>Remittance or Allotment Amount</span>, children: data.AllotAmount || '' },
        //User === 'Credit' && { key: '55', label: <span className={`font-semibold ${data.SpIncome ? 'text-black' : 'text-red-600'}`}>Remittance or Allotment Schedule</span>, children: data.ofwallotsched || '' },
        (User === 'Credit' || User === 'Lp') && { key: '58', label: <span className={`font-semibold ${data.AllotChannel ? 'text-black' : 'text-red-600'} w-[11rem]`}>Remittance / Allotment Channel (Gcash, Bank etc)</span>, children: AllotChannel().find(AllotChannel => AllotChannel.value === data.AllotChannel)?.label || '' },
    ...
    if (User === 'MARKETING' || ((User === 'Credit' || User === 'Lp') && (data.loanProd === '0303-WA' || data.loanProd === '0303-WL'))) {
        if (User !== 'LC') {
            eduItems.push(
                { key: '59', label: <span className={`font-semibold ${data.ofwHighestEdu ? 'text-black' : 'text-red-600'} w-[8rem]`}>Highest Educational Attainment</span>, children: EducationalAttainment().find(Edu => Edu.value === data.ofwHighestEdu)?.label || '' },
    ...
    const vesselItems = [
        (User === 'Credit' || User === 'Lp') && { key: '62', label: <span className={`font-semibold ${data.VesselName ? 'text-black' : 'text-red-600'}`}>Name of Vessel</span>, children: data.VesselName || '' },
            ...
            <Descriptions title={<div className="text-center">
                ...
                <div className="mt-2 flex justify-center">
                    {GetData('ROLE').toString() === '60' && (
                        <a
                            href={`https://www.google.com/search?q=${encodeURIComponent(`${data.ofwfname} ${data.ofwmname} ${data.ofwlname}`)}`}
                            target="_blank"
