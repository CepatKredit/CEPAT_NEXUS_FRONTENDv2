...
function ViewOfwDetails({ data, User, RelativesCount, receive }) {
    ...
    const { data: getCountrySelect } = useQuery({
    ...
    const {data: getCurrencyList }= useQuery({
    ...
    if (data.ofwresidences === 3) {
        items.push({
            ...
            children: data.ofwrent ? formatNumberWithCommas(formatToTwoDecimalPlaces(data.ofwrent)) : '',
        });
    ...
    const addressItems = [
        ...
        User !== 'LC' && {key: '31', label: (<span className={`font-semibold ${data.ofwlosMonth || data.ofwlosYear ? 'text-black' : 'text-orange-500'}`}> Length of Stay </span>),children: `${data.ofwlosYear ? `${data.ofwlosYear} years` : ''}${data.ofwlosMonth || data.ofwlosMonth === 0 ? ` / ${data.ofwlosMonth} months` : ''}`},
    ...
    const idItems = [
        ...
        User !== 'LC' && { key: '43', label: <span className={`font-semibold ${data.ofwcountry ? 'text-black' : 'text-orange-500'}`}>Country of Employment</span>, children: getCountrySelect?.find(country => country.code === data.ofwcountry || country.label === data.ofwcountry)?.description || '' },
        User !== 'LC' && { key: '44',label: (<span className={`font-semibold ${data.ofwjobtitle ? 'text-black' : 'text-orange-500'}`}>{(User === 'Credit' || User === 'Lp') ? 'Job Category' : 'Job Title / Position'}</span>),
        ...
        (User !== 'LC' && ((User !== 'Credit' && User !== 'Lp') || ((User === 'Credit' || User === 'Lp') && (data.loanProd === '0303-WA' || data.loanProd === '0303-WL')))) && {key: '47',label: <span className={`font-semibold ${data.ofwcompany ? 'text-black' : 'text-orange-500'}`}>
        ...
        (User === 'Credit' || User === 'Lp') && {  key: '47', label: <span className={`font-semibold ${data.FCurrency ? 'text-black' : 'text-orange-500'}`}>Foreign Currency</span>, children: data.FCurrency? `${getCurrencyList?. find(idtype => idtype.currencyCode === data.FCurrency)?.currencyName}`: ''},
