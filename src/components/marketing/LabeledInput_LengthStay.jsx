...
function LabeledInput_LengthStay({
  ...
  const [getValueM, setValueM] = React.useState(
    value_month ? value_month.toString() : "0"
  );
  const [getValueY, setValueY] = React.useState(
    value_year ? value_year.toString() : "0"
  );
  ...
  function onChangeValueMos(e) {
    ...
    if (
      (inputValue === "" || getValueY === "") &&
      /^[0-9]*$/.test(inputValue)
    ) {
      setValueM(inputValue); // Display 0 if the input is empty
      debReceiveM(0);
