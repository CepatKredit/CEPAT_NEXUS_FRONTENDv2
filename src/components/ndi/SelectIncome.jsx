...
function SelectIncome({ data, event, excludeItems }) {
    const [api, contextHolder] = notification.useNotification();
    const NDIIncomeListQuery = useQuery({
        queryKey: ['NDIIncomeList'],
        queryFn: async () => {
            const result = await GET_LIST(`/GroupGet/G24N/${parseInt(23)}`);
            return result.list;
        },
    ...
    const onClickAdd = useMutation({
        mutationFn: async () => {
            await axios.post(`/GroupPost/P74ANS/${parseInt(21)}/${newValue}/${jwtDecode(token).USRID}`)
                .then((result) => {
                    setValue('');
    ...
    const filteredOptions = NDIIncomeListQuery.data?.filter(item => !excludedIds.has(item.id))
        .map(item => ({
            key: item.id,
            value: item.id,
            label: item.name,
    ...
    return (
        <div className='w-[12rem]' ref={dropdownRef}>
            {contextHolder}
            <Select
                ref={selectRef}
                ...
                dropdownRender={(menu) => (
                    <div ref={dropdownRef}>
                        ...
                        <Space>
                            ...
                            />
                            <Tooltip placement='bottom' title={'Add Income'}>
                                <Popconfirm
                                    title='Add Record'
