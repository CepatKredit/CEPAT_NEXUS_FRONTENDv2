...
function ACB({ activeKey, onValueChange, onOtherIncome, onOtherExpense, InitialOtherIncome, InitialOtherExpense, data, setMiscellanious }) {
    ...
    const NdiDataQuery = useQuery({
        ...
        queryFn: async () => {
            ...
            await axios.get(`/GroupGet/G109ND/${loanApp}/${borrow}/${type}/${user}/${date}`)
                .then((result) => {
    ...
    React.useEffect(() => { onValueChange(3, getValue); onOtherIncome(3, getOtherIncome); onOtherExpense(3, getOtherExpense) }, [getValue, getOtherIncome, getOtherExpense])
    ...
    return (
        <>
            <SectionHeader title='NDI Additional Co-Borrower' />
