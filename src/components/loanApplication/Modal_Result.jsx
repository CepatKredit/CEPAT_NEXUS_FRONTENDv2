...
function Modal_Result({
  ...
  //   };
  //   const result = await POST_DATA("/v1/POST/P47CL", checkLoan);
  ...
  const checkLoanMutation = useMutation({
    mutationFn: async () => {
      ...
      return await POST_DATA("/v1/POST/P47CL", checkLoan);
    },
    onSuccess: (result) => {
      if (result.list.length === 0) {
        ClickLoan.mutate();
  ...
  const ClickLoan = useMutation({
    mutationFn: async () => {
      const endpoint = direct ? "/v1/POST/P52ADC" : "/v1/POST/P51ALCC";
