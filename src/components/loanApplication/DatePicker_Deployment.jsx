...
function DatePicker_Deployment({
  ...
  let getItem = dayjs(getAppDetails.loanDateDep) || "";
  ...
  function selectedDate(date) {
    ...
    if (!dayjs().isBefore(date, "day") && !dayjs().isSame(dayjs(date), "day")) {
      ...
      setStatus("error");
      setIcon(true);
      updateAppDetails({ name: "loanDateDep", value: null });
    } else {
  ...
  React.useEffect(() => {
    if (rendered) {
      if (
        !dayjs().isBefore(dayjs(getAppDetails.loanDateDep), "day") &&
        !dayjs().isSame(dayjs(getAppDetails.loanDateDep), "day")
      ) {
        setStatus("error");
        setIcon(true);
      } else {
