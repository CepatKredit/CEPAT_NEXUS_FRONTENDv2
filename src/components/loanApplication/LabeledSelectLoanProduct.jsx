...
function LabeledSelectLoanProduct({
  ...
  const { getStatus, getIcon, onChange, onBlur } = useSelectValidation(getAppDetails.loanProd, rendered, (e) => updateAppDetails({ name: 'loanProd', value: e }));
    ...
    //could be separated 
    const options = ProductSelect.data?.map(x => ({ value: x.code, label: x.description }));
    const suffixIcon = getIcon ? (getStatus === 'error'
