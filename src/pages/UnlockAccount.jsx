...
import { Card, Input, Button, ConfigProvider, notification, Spin, message } from 'antd'
...
function UnlockAccount() {
    ...
    async function checkURLValidity() {
        await axios.post(`/api/v1/POST/P93VU/${id}`)
            .then((result) => {
    ...
    const onClickUnlock = useMutation({
        mutationFn: async () => {
            ...
            let ctr_password = 0;
            await axios.post(`/api/v1/POST/P94CP/${getAccount.Id}`)
                .then((result) => {
            ...
            if (ctr_password === 0) {
                await axios.post('/api/v1/POST/P97RP', data)
                    .then((result) => {
    ...
    return (
        <div>
            {contextHolder}
            <Spin indicator={<LoadingOutlined style={{ fontSize: 100, color: 'white' }} spin />} spinning={onClickUnlock.isPending} fullscreen />
            {
                getStatus === 1
                    ? (
                        <div className='h-[100vh] w-[100vw]'>
                            <div className='flex h-screen'>
                                <Card title={'Unlock Account'} className='w-[600px] h-[450px] shadow-xl m-auto'>
                                    <div className={'w-full py-3'}>
                                        ...
                                        <div>
                                            <Input.Password style={{ width: '100%' }}
                                                ...
                                                status={getData.password === getData.rePassword ? '' : 'error'} />
                                        </div>
                                    </div>
                                    ...
                                    <center>
                                        <ConfigProvider theme={{ token: { colorPrimary: '#6b21a8' } }}>
                                            <Button className='bg-[#3b0764] mt-5 w-[100px]' size='large' type='primary' onClick={() => { onClickUnlock.mutate() }}
