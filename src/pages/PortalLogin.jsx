...
import { Button, Input, ConfigProvider, Space, notification, Spin, Divider } from 'antd'
...
import { viewModal, viewResetPasswordModal, viewForgotPasswordModal, viewUnlockAccountModal } from '@hooks/ModalController';
...
function PortalLogin() {
    ...
    const getModalResetStatus = viewResetPasswordModal((state) => state.modalStatus)
    ...
    return (
        <div className='flex flex-wrap bg-cover bg-no-repeat h-[100vh] w-[100vw]' style={{ backgroundImage: `url(${FullScreenBackground})` }}>
            {contextHolder}

            <ResponsiveModal showModal={getModalStatus} closeModal={() => { setModalStatus(false) }}
                modalWidth={'600px'} modalTitle={<><a href="
        ...
        https://www.positivessl.com/images/seals/positivessl_trust_seal_sm_124x32.png"
                    /></a></>} contextHeight={'h-[500px]'} contextInside={(<><TrackLoanApp /></>)} />

            <ResponsiveModal showModal={getModalResetStatus} closeModal={() => { setModalResetStatus(false) }} modalTitle={<span>Reset Temporary Password</span>}
                modalWidth={'400px'} contextHeight={'h-[420px]'} contextInside={(<><ResetPassword /></>)} />

            <ResponsiveModal showModal={getOTPStatus} closeModal={() => { onClickCancelOTP() }} modalWidth={'500px'} contextHeight={'500px'} contextInside={<ValidationOTP username={getAccount.Username}
                accessList={getAccessList} />} />

            <ResponsiveModal showModal={getForgotStatus} closeModal={() => { setForgotStatus(false) }} modalWidth={'400px'} contextHeight={'250px'}
                modalTitle={<span>Forgot Password</span>} contextInside={<><ForgotPassword /></>} />

            <ResponsiveModal showModal={getUnlockStatus} closeModal={() => { setUnlockStatus(false) }} modalWidth={'400px'} contextHeight={'250px'}
                modalTitle={<span>Unlock Account</span>} contextInside={<><UnlockAccount /></>} />

            <Spin indicator={<LoadingOutlined style={{ fontSize: 100, color: 'white' }} spin />} fullscreen
                spinning={onClickLogin.isPending || PasswordMatch.isPending || PasswordNotMatch.isPending} />
            <div className='h-[100%] w-[67%]'>
                <div className='pt-[3%] ml-[3%] w-[150px]'>
                    <a href="https://secure.trust-provider.com/ttb_searcher/trustlogo?v_querytype=W&v_shortname=POSDV&v_search=https://ckfi.live/cepat-portal/index.php&x=6&y=5"
                        target="_blank"><img className='w-[150px]' src="https://www.positivessl.com/images/seals/positivessl_trust_seal_sm_124x32.png"
                        /></a>
                </div>
                <center>
                    ...
                    <div>
                        <ConfigProvider theme={{ token: { colorPrimary: '#b3b2b1', borderRadius: 100, borderRadiusLG: 100, borderRadiusSM: 100 } }}>
                            <Button className='mt-5 text-2xl font-semibold text-black w-[400px] h-[50px] bg-[#ffffff]' size='large' type='primary'
                                onClick={() => { setModalStatus(true) }}
                                onKeyUp={(e) => { e.key === 'enter' ? setModalStatus(true) : '' }}>LOAN APPLICATION STATUS</Button>
                        </ConfigProvider>
                    </div>
                </center>
            </div>
            <div className='h-[100%] w-[30%] bg-left drop-shadow-[0_5px_5px_rgba(0,0,0,.5)]' style={{ backgroundImage: `url(${LoginBackground})` }}>
                <div className='h-[100%] w-[100%] bg-stone-800/70'>
                    <center>
                        <div className='pt-[50%] h-[500px]'>
                            <div className='text-2xl font-normal text-stone-100'>
                                <div>
                                    For registered personnel and
                                </div>
                                <div>
                                    loan consultants, please log in below:
                                </div>
                            </div>
                            <div className='w-[90%] pt-2'>
                                <div className='pt-2'>
                                    <ConfigProvider theme={{ token: { borderRadius: 100, borderRadiusLG: 100, borderRadiusSM: 200 } }}>
                                        <Input size='large'
                                            name='Username'
                                            placeholder='Email Address'
                                            prefix={<Space.Compact className='px-2 w-[40px]  text-stone-400'><IoMail style={{ fontSize: 20 }} /></Space.Compact>}
                                            autoComplete='off'
                                ...
                                <div className='pt-2'>
                                    <ConfigProvider theme={{ token: { borderRadius: 100, borderRadiusLG: 100, borderRadiusSM: 200 } }}>
                                        <Input.Password
                                            ...
                                            prefix={<Space.Compact className='px-2 w-[40px] text-stone-400'><FaKey style={{ fontSize: 20, }} /></Space.Compact>}
                                            autoComplete='off'
                                            onChange={handleChange}
                                ...
                                <div className='flex place-content-between'>
                                    <div className='pt-2 pr-2 h-[10px] pl-2 cursor-pointer' onClick={() => { setUnlockStatus(true) }}>
                                        <span className='font-bold text-sky-400 hover:text-cyan-300 text-base' >Unlock Account</span>
                                    </div>
                                    <div className='pt-2 pr-2 h-[10px] cursor-pointer' onClick={() => { setForgotStatus(true) }}>
                                        <span className='font-bold text-sky-400 hover:text-cyan-300 text-base' >Forgot Password?</span>
                                    </div>
                                </div>
                                <div className='text-center pt-9'>
                                    <ConfigProvider theme={{ token: { colorPrimary: '#31b234', borderRadius: 100, borderRadiusLG: 100, borderRadiusSM: 100 } }}>
                                        <Button className='text-2xl font-semibold w-[200px] h-[50px] bg-[#31b234]' onClick={() => { onClickLogin.mutate() }}
                                            onKeyUp={(e) => { e.key === 'enter' ? onClickLogin.mutate() : '' }}
            ...
            <div className='flex flex-wrap w-[100%] mt-[-180px]'>
                ...
                <div className='pl-[3%] text-white w-[30%]' style={{ fontSize: 8 }}>
                    <strong>
                        <h5>CEPAT KREDIT FINANCING INC.</h5><br />
                        <article>
                            SEC Registration No.: CS201909976<br />
                            CEPAT KREDIT FINANCING INC. is regulated by the Securities and Exchange Commission (SEC)<br />
                            SEC Contact Information : (02)8818-5990 / cgfd-flcd@sec.gov.ph.<br />
                            Certificate of Authority to Operate: No. 1223<br /><br />
                        </article>
                        <article>
                            ADVISORY: Welcome to CEPAT KREDIT FINANCING’s portal. By accessing this website, you agree
                            to be governed by the Terms and Conditions and Data Privacy Policy herein set forth. If you find the
                            Terms and Conditions and data privacy policy unacceptable, kindly discontinue accessing the website.
                            Before proceeding with your loan transaction, we advise you to study the Terms and Conditions in the disclosure statements.<br /><br />
                        </article>
                    </strong>
                </div>
                    ...
                    <strong>
                        ...
                        <article>
                            <div className='flex'>
                                ...
                                <span>
                                    Room 2402 Jollibee Plaza, Emerald Ave., San Antonio, Pasig, Metro Manila
                                </span>
                            </div>
                            <div className='flex'>
                                <span className='pt-[2px] pr-[5px]'>
                            ...
                            <div className='flex'>
                                <span className='pt-[3.5px] pr-[5px]'>
                                    {<IoMail />}
                                </span>
                                <span>
                                    customerservice@cepatkredit.com
                                </span>
                            </div>
                            <div className='flex'>
                                <span className='pt-[3px] pr-[5px]'>
                                    {<FaCalendar />}
                                </span>
                                <span>
