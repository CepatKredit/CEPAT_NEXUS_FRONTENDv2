...
function NetProceeds({ data }) {
  ...
  const nameListOptions = React.useMemo(() => NameList(getAppDetails), [getAppDetails]);
  ...
  React.useEffect(() => {
    console.log("NET PROCEEDS DATA", NameList(getAppDetails));
  }, []);
  ...
  async function AddNP() {
    ...
    await axios
      .post("/createDisbursement", container)
      .then((result) => {
              ...
              <div className="w-[15rem]">
                ...
                <Select
                  ...
                  onChange={(value) => {
                    ...
                    const selectedOption = nameListOptions.find((item) => item.name === value);
                    if (selectedOption) {
