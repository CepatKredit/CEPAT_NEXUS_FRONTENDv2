...
function NetProceeds({ data }) {
    ...
    const { getBank, getPurpose, SET_LOAN_APPLICATION_NUMBER, GET_TOTAL_AMOUNT, GET_REFRESH_LAN, SET_REFRESH_LAN } = useDataContainer()
    ...
    React.useEffect(() => { SET_LOAN_APPLICATION_NUMBER(data.LAN.props.children); ComputeRemaining(); }, [data])

    React.useEffect(() => { ComputeRemaining(); }, [GET_REFRESH_LAN, GET_TOTAL_AMOUNT])
    const [getRemaining, setRemaining] = React.useState('')
    function ComputeRemaining() {
        let value = parseFloat(removeCommas(data.AA)) - parseFloat(removeCommas(GET_TOTAL_AMOUNT))
        setRemaining(value)
    }
    ...
    function onChangeAmount(e) {
        ...
        if (getData.PaymentType === 'PESONET' && parseFloat(num) >= parseFloat(getRemaining)) {
            setData({ ...getData, Amount: formatNumberWithCommas(getRemaining.toString()) })
        }
        else if (getData.PaymentType === 'INSTAPAY' && parseFloat(num) >= parseFloat(getRemaining)) {
    ...
    async function AddNP() {
        ...
        await axios.post('/createDisbursement', container)
            .then((result) => {
                queryClient.invalidateQueries({ queryKey: ['DisbursementListQuery', data.LAN.props.children] }, { exact: true })
                SET_REFRESH_LAN(1)
                queryClient.invalidateQueries({ queryKey: ['PRELOAD_DISBURSEMENT', data.LAN.props.children] }, { exact: true })
                ...
                <div className='w-[29rem]'>
                    <Space>
                        <div className='w-[15rem]'>
                            <Radio.Group disabled={formatNumberWithCommas(truncateToDecimals(getRemaining).toString()).toString() === '0.00'} onChange={(e) => {
                                setData({ ...getData, PaymentType: e.target.value });
                                if (truncateToDecimals(removeCommas(getData.Amount)) >= 50000 && e.target.value === 'INSTAPAY') { setData({ ...getData, PaymentType: e.target.value, Amount: '50,000.00' }) }
                        ...
                        <div className='w-[15rem]'>
                            <div className='font-bold'>Remaining to Disburse: {formatNumberWithCommas(parseFloat(getRemaining).toFixed(2).toString())}</div>
                        </div>
                    </Space>
                    <Space>
                        <div>
                            ...
                            <div className='w-[15rem]'>
                                <Input className='w-full' disabled={!getData.PaymentType || formatNumberWithCommas(parseFloat(getRemaining).toFixed(2).toString()).toString() === '0.00'}
                                    value={getData.FirstName} onChange={(e) => { setData({ ...getData, FirstName: e.target.value }) }} />
                            </div>
                        </div>
                        <div>
                            ...
                            <div className='w-[15rem]'>
                                <Input className='w-full' disabled={!getData.PaymentType || formatNumberWithCommas(parseFloat(getRemaining).toFixed(2).toString()).toString() === '0.00'}
                                    value={getData.LastName} onChange={(e) => { setData({ ...getData, LastName: e.target.value }) }} />
                            </div>
                        </div>
                    </Space>
                    <Space>
                        <div>
                            ...
                            <div className='w-[15rem]'>
                                <Select className='w-full' value={getData.BankName || undefined} disabled={!getData.PaymentType ||
                                    formatNumberWithCommas(parseFloat(getRemaining).toFixed(2).toString()).toString() === '0.00'}
                        ...
                        <div>
                            ...
                            <div className='w-[15rem]'>
                                <Input className='w-full' disabled={!getData.PaymentType || formatNumberWithCommas(parseFloat(getRemaining).toFixed(2).toString()).toString() === '0.00'}
                                    value={getData.BankAcctNo} onChange={(e) => { setData({ ...getData, BankAcctNo: e.target.value }) }} />
                            </div>
                        </div>
                    </Space>
                    <Space>
                        ...
                        <Space>
                            <div>
                                ...
                                <div className='w-[10.6rem]'>
                                    <Input className='w-full' disabled={!getData.PaymentType || formatNumberWithCommas(parseFloat(getRemaining).toFixed(2).toString()).toString() === '0.00'}
                                        value={getData.Amount} onBlur={(e) => { onBlurAmount(e) }} onChange={(e) => { onChangeAmount(e) }} />
                                </div>
                            </div>
                            <Button disabled={!getData.FirstName || !getData.LastName || !getData.BankName || !getData.BankAcctNo || getData.Amount === '0.00' || !getData.Purpose ||
                                formatNumberWithCommas(parseFloat(getRemaining).toFixed(2).toString()).toString() === '0.00'}
                                className='mt-[1.6em]' type='primary' onClick={() => { AddNP() }}>Save</Button>
                        </Space>
                    </Space>
                </div>
                <div className='px-1 w-[70%]'>
                    <DisbursementList LAN={data.LAN.props.children} type={'NP'} bankList={getBank} DisburseAmount={data.AA} />
                </div>
            </div>
        </div>
    )
