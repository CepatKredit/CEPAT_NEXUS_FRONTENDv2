...
function BatchList() {
    ...
    const GetBatchList = useQuery({
        ...
        queryFn: async () => {
            const result = await GET_LIST(`/api/v1/GET/G101BL/${jwtDecode(token).USRID}`);
            return result.list;
        },
        enabled: true,
    ...
    async function UpdateStatus(res,id,filename) {
        try {
            res.list.forEach(async (x) => {
                await axios.post(`/api/v1/POST/P125USD/${x.id}/${jwtDecode(token).USRID}/${'0'}/${'0'}`);
            });
            await axios.post(`/api/v1/POST/P126UFB/${id}/${filename}`);
        } catch (error) {
            console.log(error)
        }
    ...
    async function genTextSBD(v,filename) {
        let result = [];
        try {
            result = await GET_LIST(`/api/v1/GET/G103BD/${v.id}`);
        } catch (error) {
