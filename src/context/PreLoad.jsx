...
function PreLoad({ children }) {
  ...
  const getPurposeList = useQuery({
    ...
    queryFn: async () => {
      const result = await GET_LIST(`/api/v1/GET/G99PL`);
      let dataContainer = [];
      result.list?.map((x) => {
  ...
  const getBankList = useQuery({
    ...
    queryFn: async () => {
      const result = await GET_LIST(`/api/v1/GET/G100PT`);
      let dataContainer = [];
      result.list?.map((x) => {
        dataContainer.push(x);
      });
  ...
  const getDisbursementList = useQuery({
    ...
    queryFn: async () => {
      ...
      const result = await GET_LIST(
        `/api/v1/GET/G106DL/${GET_LOAN_APPLICATION_NUMBER}/${"NP"}`
      );
      result.list?.map((x) => {
  ...
  const TileCountListQuery = useQuery({
    ...
    queryFn: async () => {
      const result = await GET_LIST(`/api/v1/GET/G98TC`);
